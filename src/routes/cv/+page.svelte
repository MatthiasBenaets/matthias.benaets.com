<script lang="ts">
	...
	let exp = $state(experience);

	function toggleVisible(index: number) {
		exp[index].visible = !exp[index].visible;
	}
...
<Seo
	title="CV"
	description="Curriculum Vitae Matthias Benaets"
	keywords="CV Curriculum Vitae Resume Matthias Benaets"
	canonical="https://benaets.com/cv"
...
{#if ready}
	...
	<div class="sm:w-[640px] mx-auto px-5 sm:px-0">
		...
		{#each exp as { period, name, company, link, tasks, visible }, index}
			<div
				class="flex flex-col sm:flex-row text-sm mb-10"
				in:fly|global={{ x: -30, duration: 300, delay: 400 + 100 * index, easing: backOut }}
			>
				<div class="w-3/12">
		...
		{#each education as { period, name, honor, school, link }, index}
			<div
				class="flex flex-col sm:flex-row text-sm mb-10"
				in:fly|global={{ x: -30, duration: 300, delay: 800 + 100 * index, easing: backOut }}
			>
